...
從一開始 CDN 載入就直接撞牆，好險最後用本地端方法有成功連上，不然我可能會放棄挑戰 Lv2。<br>
按鈕的文字寫入功能卡關了一個晚上，甚至連睡覺都在想要怎麼寫，不斷嘗試不同方法想讓我的程式不要落落長。<br>
這時想到可以用按鈕的序號來取陣列的值，我原本寫 `.indexof( );` 但console 一直無情地告訴我 Jquery 沒這個語法，最後解題的關鍵是 `.index( );` 這個 Jquery 的索引的寫法。只能說，初學者不要偷懶要好好看[官方文件](https://api.jquery.com/index/)啊~
...
## 感謝六角學院
最後的最後要感謝六角學院的校長侑杰和強大的助教團隊！<br>
當初在網路上找學習資源時，正好看到六角即將舉辦體驗營，其中最吸引我的是作業批改以及問到飽的資源，沒想到，只有六角能超越六角，我從這裡學到的比想像中多更多！<br>
除了影音課程、小節作業之外，每日任務和各種加碼教學、熱絡的 Discord 討論區，讓原本的課程就像是解壓縮一樣，越來越豐富，這一個多月來收穫滿滿滿！<br>
想到今年三月中的我一個程式碼都不會，就這樣慢慢地能自己用鍵盤敲出的網頁，真的成就感爆棚 ~~
